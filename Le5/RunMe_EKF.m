...
model.F = @(x) [0.99 * x(1) + 0.2 * x(2);...
                -0.1 * x(1) + 0.5 * x(2) / (1 + x(2)^2)];
model.H = @(x) x(1) - 3 * x(2) + sqrt(model.R) * rand;

model.Fx = @(x) [0.99, 0.2; ...
                 -0.1, 0.5 / (x(2)^2 + 1) - x(2)^2 / (x(2)^2 + 1)^2];
model.Hx = @(x) [1 -3];
...
data.y = zeros(1,N);
data.x(:,1) = x0 + model.B * sqrt(1) * rand;
data.y(:,1) = model.H(data.x);
for ii = 2:N
    data.x(:,ii) = model.F(data.x(:,ii-1)) + model.B * rand;
    data.y(:,ii) = model.H(data.x(:,ii));
end
...
nexttile();
plot(1:N,data.x(1,:)); box off; hold on
plot(1:N,xhatEKF(1,:)); box off; hold on
...
nexttile();
plot(1:N,data.x(2,:)); box off; hold on
plot(1:N,xhatEKF(2,:)); box off; hold on
legend('sim','EKF'); legend('Interpreter','latex');
ylabel('$x_1$','Interpreter','latex');
xlabel('$N$','Interpreter','latex');
...
if saveplots
    print -dpdf ../doc/figures/ex5_MHE_EKF.pdf
end

%% MSE
MSE.EKF = sum(sqrt((data.x - xhatEKF).^2), 2);
MSE.MHE = sum(sqrt((data.x - xhat_MHE).^2), 2);
...
for ii = 1:1:T-1
    opti.subject_to(X(:,ii+1) == model.F(X(:,ii)) + B * w(ii)); 
    vk(ii) = y(ii) - model.H(X(:, ii));
end
...
% fetching the solutions from the OCP
    w_op = sol.value(w);
    x_temp = sol.value(x0_hat);
% simulating the system
for ii = 1:1:T-1
    x_temp = model.F(x_temp) +B*w_op(ii);
end
