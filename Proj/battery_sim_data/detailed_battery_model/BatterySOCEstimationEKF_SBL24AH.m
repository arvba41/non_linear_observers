...
datasheet_data = Battery_SBL24AH_cell_data();

% extracting the battery data at 25 degC 
vocv_ds = datasheet_data.OCV_vec; 
qsoc_ds = datasheet_data.SOC_vec;
q0_ds   = datasheet_data.AH;
qsoc_R0_ds = datasheet_data.SOC_R0_vec;
R0_ds   = datasheet_data.R0_vec;

params.ocv  = SplineInterpol(qsoc_ds / 100 * q0_ds, vocv_ds);
params.R0   = SplineInterpol(qsoc_R0_ds / 100 * q0_ds, R0_ds);
...
plot(qsoc_R0_ds, R0_ds * 1e3, 'x'); box off; 
...
clear vocv_ds qsoc_ds qsweep q0_ds h R0_ds qsoc_R0_ds% clearing data
%% simulating the data in simulink
simdata = sim("sim_mld_batt_SBL24AH.slx");
...
% for y=1:12 
    JSONFILE_name = sprintf('simdata.json'); 
    fid = fopen(JSONFILE_name,'w');
    s = struct("Time", simdata.tout, "Current", simdata.Current, "Voltage", simdata.Voltage, "SOC", simdata.Real_SOC); 
...
yline(3,'k--',{'$\pm$ 3\%'},'Interpreter','latex','LabelHorizontalAlignment','left');
